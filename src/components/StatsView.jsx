...
const StatsView = () => {
  ...
  return (
    <div className="animate-fade-in" style={{ width: '100%', maxWidth: '800px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {/* Üstteki Özet Kartları (Dashboard Summary) */}
      ...
      {/* Ana Çizgi/Sütun Analiz Grafiği */}
      <div className="panel" style={{ padding: '32px' }}>
         <h2 className="panel-title" style={{ marginBottom: '40px' }}><Activity size={20}/> Focus Analysis (Last 7 Days)</h2>
               ...
               return (
                  <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', gap: '12px', height: '100%' }}>
                     ...
                     <div style={{ 
                        ...
                        {/* Çubuğun üstündeki dakika yazar */}
                        {data.minutes > 0 && (
                          <span style={{ position: 'absolute', bottom: `${heightPercent}%`, marginBottom: '8px', fontSize: '0.7rem', fontWeight: 800, color: isToday ? 'var(--primary)' : 'var(--text-muted)' }}>
                             {data.minutes}m
                          </span>
                        )}
                        <div style={{
                           ...
                           background: isToday ? 'var(--primary)' : 'var(--primary-transparent)',
                           borderRadius: '12px 12px 0 0',
                           transition: 'height 1s cubic-bezier(0.4, 0, 0.2, 1)',
                           boxShadow: isToday ? '0px -4px 20px var(--primary-transparent)' : 'none'
                        }}></div>
                     </div>
                     {/* Alt Etiketler */}
                     <div style={{ fontSize: '0.75rem', fontWeight: isToday ? 800 : 600, color: isToday ? 'var(--text-strong)' : 'var(--text-muted)' }}>
                        {data.label}
                     </div>
                  </div>
               )
         ...
         <div style={{ textAlign: 'center', marginTop: '16px', fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600 }}>
            *Chart values show focus session durations in minutes.
         </div>
      </div>
      ...
      {/* Session History Log */}
      <div className="panel" style={{ padding: '32px' }}>
         <h3 className="panel-title" style={{ marginBottom: '24px' }}><Clock size={20}/> Session History</h3>
         
         {sessionHistory.length === 0 ? (
           <div style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.9rem', padding: '20px' }}>
              No sessions completed yet. Start a timer to see your history here!
           </div>
         ) : (
